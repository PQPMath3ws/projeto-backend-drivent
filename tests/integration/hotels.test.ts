...
describe('GET /hotels', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 404 when not have hotels', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
...
describe('GET /hotels/:hotelId', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 400 for invalid hotel id', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithIncludesHotel();
      ...
      const response = await server.get(`/hotels/${hotel.id + 4}`).set('Authorization', `Bearer ${token}`);
